...
double SunSet::calcJD(int y, int m, int d) {
	...
	double JD = floor(365.25 * (y + 4716)) + floor(30.6001 * (m + 1)) + d + B
			- 1524.5;
	return JD;
}
...
double SunSet::calcSunrise_setUTC(int event) {
	...
	hourAngle =
			(event == RISE) ?
					calcHourAngleSunrise(latitude, solarDec) :
					calcHourAngleSunset(latitude, solarDec);
...
double SunSet::udaysFromSid(double jd_fromEpoch, double sidTime,
		double longitude) {
...
double SunSet::starRiseSetTimeSidereal(double ra, double decl, double lat,
		int event) {

	return (event == SET) ?
			ra + acos(-tan(decl) * tan(lat)) : ra - acos(-tan(decl) * tan(lat));;
}
int SunSet::starRise_SetTime(double jd, double ra_deg, double decl_deg, double lat_deg, double longitude_deg, int event) {
    double tzOffset = 3.0;
	...
	double eventTime = adjustRad(starRiseSetTimeSidereal(ra, decl, lat, event)); //:adjustRad(starRiseTimeSidereal(ra,decl,lat));
